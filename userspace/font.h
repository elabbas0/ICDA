#ifndef USERSPACE_FONT_H
#define USERSPACE_FONT_H

/* Bitmap font — identical data to kernel/drivers/display/font.h */
...
static inline void font_draw_char(uint32_t *pixels, int buf_w, int buf_h,
                                  int pitch_pixels,
                                  int x, int y, char c,
                                  uint32_t fg, uint32_t bg) {
    unsigned char uc = (unsigned char)c;
    if (uc < FONT_FIRST || uc > FONT_LAST) uc = '?';
    const unsigned char *glyph = font_data[uc - FONT_FIRST];
