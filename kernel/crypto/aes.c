#include "aes.h"

static const uint8_t SBOX[256] = {
    
};

static const uint8_t RSBOX[256] = {
    
};

static const uint8_t RCON[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

static uint8_t xtime(uint8_t a) {
    return (uint8_t)((a << 1) ^ (((a >> 7) & 1) * 0x1b));
}

static uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t r = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) r ^= a;
        uint8_t h = (uint8_t)(a & 0x80);
        a <<= 1;
        if (h) a ^= 0x1b;
        b >>= 1;
    }
    return r;
}

static void sub_word(uint8_t *w) {
    w[0] = SBOX[w[0]]; w[1] = SBOX[w[1]];
    w[2] = SBOX[w[2]]; w[3] = SBOX[w[3]];
}

static void rot_word(uint8_t *w) {
    uint8_t t = w[0];
    w[0] = w[1]; w[1] = w[2]; w[2] = w[3]; w[3] = t;
}

void aes128_expand_key(const uint8_t key[AES128_KEY_SIZE], uint8_t rk[176]) {
    for (int i = 0; i < 16; i++) rk[i] = key[i];
    for (int i = 4; i < 44; i++) {
        uint8_t *prev = rk + (i-1)*4;
        uint8_t *cur = rk + i*4;
        if (i % 4 == 0) {
            uint8_t tmp[4] = {prev[0], prev[1], prev[2], prev[3]};
            rot_word(tmp);
            sub_word(tmp);
            tmp[0] ^= RCON[i/4];
            cur[0] = rk[(i-4)*4] ^ tmp[0];
            cur[1] = rk[(i-4)*4+1] ^ tmp[1];
            cur[2] = rk[(i-4)*4+2] ^ tmp[2];
            cur[3] = rk[(i-4)*4+3] ^ tmp[3];
        } else {
            cur[0] = rk[(i-4)*4] ^ prev[0];
            cur[1] = rk[(i-4)*4+1] ^ prev[1];
            cur[2] = rk[(i-4)*4+2] ^ prev[2];
            cur[3] = rk[(i-4)*4+3] ^ prev[3];
        }
    }
}

static void add_round_key(uint8_t state[AES_BLOCK_SIZE], const uint8_t rk[176], int round) {
    for (int i = 0; i < 16; i++) state[i] ^= rk[round*16 + i];
}

static void sub_bytes(uint8_t state[AES_BLOCK_SIZE]) {
    for (int i = 0; i < 16; i++) state[i] = SBOX[state[i]];
}

static void inv_sub_bytes(uint8_t state[AES_BLOCK_SIZE]) {
    for (int i = 0; i < 16; i++) state[i] = RSBOX[state[i]];
}

static void shift_rows(uint8_t state[AES_BLOCK_SIZE]) {
    uint8_t t;
    t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = t;
}

static void inv_shift_rows(uint8_t state[AES_BLOCK_SIZE]) {
    uint8_t t;
    t = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = t;
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = t;
}

static void mix_columns(uint8_t state[AES_BLOCK_SIZE]) {
    for (int i = 0; i < 4; i++) {
        int j = i * 4;
        uint8_t a0 = state[j], a1 = state[j+1], a2 = state[j+2], a3 = state[j+3];
        state[j]   = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
        state[j+1] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
        state[j+2] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
        state[j+3] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
    }
}

static void inv_mix_columns(uint8_t state[AES_BLOCK_SIZE]) {
    for (int i = 0; i < 4; i++) {
        int j = i * 4;
        uint8_t a0 = state[j], a1 = state[j+1], a2 = state[j+2], a3 = state[j+3];
        state[j]   = gf_mul(a0,14) ^ gf_mul(a1,11) ^ gf_mul(a2,13) ^ gf_mul(a3,9);
        state[j+1] = gf_mul(a0,9) ^ gf_mul(a1,14) ^ gf_mul(a2,11) ^ gf_mul(a3,13);
        state[j+2] = gf_mul(a0,13) ^ gf_mul(a1,9) ^ gf_mul(a2,14) ^ gf_mul(a3,11);
        state[j+3] = gf_mul(a0,11) ^ gf_mul(a1,13) ^ gf_mul(a2,9) ^ gf_mul(a3,14);
    }
}

void aes128_encrypt_block(const uint8_t rk[176], const uint8_t in[AES_BLOCK_SIZE], uint8_t out[AES_BLOCK_SIZE]) {
    uint8_t state[AES_BLOCK_SIZE];
    for (int i = 0; i < 16; i++) state[i] = in[i];
    add_round_key(state, rk, 0);
    for (int round = 1; round < 10; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, rk, round);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk, 10);
    for (int i = 0; i < 16; i++) out[i] = state[i];
}

void aes128_decrypt_block(const uint8_t rk[176], const uint8_t in[AES_BLOCK_SIZE], uint8_t out[AES_BLOCK_SIZE]) {
    uint8_t state[AES_BLOCK_SIZE];
    for (int i = 0; i < 16; i++) state[i] = in[i];
    add_round_key(state, rk, 10);
    for (int round = 9; round > 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, rk, round);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, rk, 0);
    for (int i = 0; i < 16; i++) out[i] = state[i];
}

void aes128_cbc_encrypt(const uint8_t rk[176], const uint8_t iv[AES_BLOCK_SIZE], const uint8_t *in, uint32_t len, uint8_t *out) {
    uint8_t block[AES_BLOCK_SIZE];
    for (uint32_t i = 0; i < AES_BLOCK_SIZE; i++) block[i] = iv[i];
    for (uint32_t offset = 0; offset < len; offset += AES_BLOCK_SIZE) {
        for (int i = 0; i < AES_BLOCK_SIZE; i++) block[i] ^= in[offset + i];
        aes128_encrypt_block(rk, block, out + offset);
        for (int i = 0; i < AES_BLOCK_SIZE; i++) block[i] = out[offset + i];
    }
}

void aes128_cbc_decrypt(const uint8_t rk[176], const uint8_t iv[AES_BLOCK_SIZE], const uint8_t *in, uint32_t len, uint8_t *out) {
    for (uint32_t offset = 0; offset < len; offset += AES_BLOCK_SIZE) {
        aes128_decrypt_block(rk, in + offset, out + offset);
        if (offset == 0) {
            for (int i = 0; i < AES_BLOCK_SIZE; i++) out[i] ^= iv[i];
        } else {
            for (int i = 0; i < AES_BLOCK_SIZE; i++) out[offset + i] ^= in[offset - AES_BLOCK_SIZE + i];
        }
    }
}
