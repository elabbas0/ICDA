---
description: Run trio debate (2x Muse Spark 1.3 + MiMo coder) then implement and verify.
agent: triobuild
---

Build the following (debate first, then implement — implementation is mandatory):
...
4. Delegate implementation to coder with the exact winning plan, review the diff, run verification (build/tests/qemu-smoke) yourself, fix fallout. Report Decision + Changes (file:line) + Verification output.
